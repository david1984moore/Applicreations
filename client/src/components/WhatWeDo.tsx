...
function ConfettiContainer({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div 
      className="confetti-container cursor-pointer" 
...
// Hammer container with trigger-based animation
function HammerContainer({ children }: { children: React.ReactNode }) {
  const [isAnimating, setIsAnimating] = useState(false);
  ...
  return (
    <div 
      className="hammer-container cursor-pointer" 
      title="Hover to see construction in action!"
      onMouseEnter={handleMouseEnter}
    >
      {/* Hammer and nail elements */}
      <div className={`hammer ${isAnimating ? 'animate-hammer' : ''}`}>
    ...
    >
      ...
      <div className="container mx-auto px-6 md:px-10 lg:px-16 xl:px-24 relative z-10">
        ...
        <div className="hidden md:flex gap-8 reveal">
          ...
          {/* Card 2: Iterate - Green Color Scheme with Hammer */}
          <div className="flex-1 bg-white/10 backdrop-blur-sm rounded-xl p-8 shadow-lg transition-all duration-300">
            <div className="flex flex-col items-center text-center">
              <HammerContainer>
